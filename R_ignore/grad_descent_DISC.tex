...
First, we find the partial derivative of \eqref{eq_loss} with respect to each of our coefficient of inbreeding parameters, $\{f_1, ..., f_K\}$, of interest. All $f$ values are essentially equivalent inside the loss function, so we can simplify this problem by working out the partial derivative for a representative $f_i$. Note that the explicit regularization term for the global migration parameter, $M$, does not contain any $f$ parameters and thus is omitted from the differential for clarity. 
...
such that $h_i$ and $g_i$ are equal due to the sequential application of \textit{logit} and \textit{logit} that result from independent reparameterizations. We then differentiate the loss function with respect to an arbitrary $c_\ell$, where $\ell \in \{1,\dots,K\}$ is defined as a separate index from $i$ and $j$, to find:
...
\subsubsection{Global Migration Rate Reparameterization and Regularization} 
...
\noindent Separately, in initial modeling efforts, we observed a pathological behavior in which if inbreeding coefficients became too large, the global migration parameter $M$, would exhibit monotonically increasing updates, $M_t$, without convergence. To promote biologic plausibility (\textit{i.e.} not allowing excessive inbreeding that corresponds to an infinite migration rate), we added an explicit L2 regularization term, $\lambda$, on the global migration parameter of interest, $M$. The $\lambda$ hyperparameter explicitly favors smaller global migration rates to avoid overfitting the exponential decay function. The regularized loss function is expressed as: 

\begin{equation} 
\label{eq_loss_regularization} 
L(\theta) = \sum_{i=1}^{K-1} \sum_{j=i+1}^K \sum_{p=1}^{P_{i,j}} (y_{p_{ij}} - r_{ij})^2 + \lambda M^2 
\end{equation} 
\noindent Accordingly, the gradient of the loss with respect to $M$ becomes: 
 \begin{equation} \
 \frac{\partial L(\theta)}{\partial M} = \sum_{i=1}^{K-1} \sum_{j=i+1}^K \sum_{p=1}^{P_{i,j}} \frac{\partial (y_{p_{ij}} - r_{ij})^2}{\partial M} + 2 \lambda M 
 \end{equation}
Note that the derivative of the L2 regularization term is considered once per global migration rate gradient calculation.
...
\begin{equation}
    ...
    = \left[
        ...
        \frac{\partial (y_{p_{ij}} - r_{ij})^2}{\partial M} + 2 \lambda M
      \right] e^m.
\end{equation}
